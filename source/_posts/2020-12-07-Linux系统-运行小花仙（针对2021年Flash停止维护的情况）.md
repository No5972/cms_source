...
在某些网站可能还能找到Linux专用的版本29的PPAPI Flash（可能也很难下载得到，可尝试百度搜索```flash player linux```，注意是32位还是64位）。如果出现了阻止加载的情况，需要将此旧版本的Flash解压然后替换到系统对应目录的同名文件（```libpepflashplayer.so```和```manifest.json```，具体目录待补充，先用```find / -name libpepflashplayer.so```确认原文件位置），即可恢复使用。
