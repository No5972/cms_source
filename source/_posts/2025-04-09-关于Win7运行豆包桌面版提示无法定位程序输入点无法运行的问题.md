...
鉴于豆包安装包现已增强了系统版本检测，本文步骤可能会比较复杂，如果读者看不懂的话，鄙人建议读者找一个身边懂电脑的朋友或者贵单位的信息部门，把本文交给他们，然后让他们指导读者来操作或者直接代为操作。

# 先上具体步骤：

一、下载并安装 VxKex NEXT 这个开源组件，可以在网上搜索这个开源组件的名字（注意找到正确的下载链接）。
二、从一个特殊的网页下载**豆包离线版安装包**（即在豆包桌面版的下载引导网页的网址后面加一串参数）：[https://www.doubao.com/download/desktop?ug_apk_token=dnsqL](https://www.doubao.com/download/desktop?ug_apk_token=dnsqL)
...
三、解压缩之后前往解压缩出来的目录，找到 ```Doubao.exe``` 这个主程序文件并右键，选择属性，VxKex NEXT 选项卡，勾选“为此程序启用 VxKex NEXT”并确定。然后看看能不能打开这个程序。最好能把这个程序创建一个桌面快捷方式（右键这个文件选择发送到 - 桌面快捷方式）。

四、如有必要，将 ```%TEMP%\doubao_ext\``` 、 ```%LOCALAPPDATA%\Doubao\Application\app\``` 目录及其子目录下所有 ```shellext.dll```、 ```shellextcore.dll``` 删除，并想办法阻止豆包在此目录及其子目录下创建这些文件（可通过设置文件夹权限等手段），防止资源管理器崩溃等异常情况。
